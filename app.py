...
def send_email_silent(pdf_bytes, client_name):
    try:
        ...
        msg = MIMEMultipart()
        msg['From'] = f"RenovationArt <{sender}>"; msg['To'] = "renovationsartstg@gmail.com"; msg['Subject'] = f"Kopia: {normalize_pl(client_name)}"
...
def create_pdf_bytes(name, addr, basket, netto, brutto, vat_val, include_mat):
    ...
    pdf.set_font("Helvetica", "", 10); pdf.set_text_color(100, 100, 100); pdf.cell(0, 5, f"Data: {datetime.date.today()}", ln=True, align="R")
...
# ==========================================
# 2. KOMPLEKSOWA BAZA USŁUG (PEŁNA LISTA)
# ==========================================
...
def load_db():
    ...
    return pd.DataFrame([
        ...
        {"Kategoria": "02. Przygotowanie", "Nazwa": "Zabezpieczenie klatki/windy/folia", "Jm": "szt", "R": 150.0, "M": 80.0},
        # 03. ŚCIANY I SUFITY
        {"Kategoria": "03. Ściany i Sufity", "Nazwa": "Gładź gipsowa (2x) + szlifowanie", "Jm": "m2", "R": 68.0, "M": 16.0},
        {"Kategoria": "03. Ściany i Sufity", "Nazwa": "Malowanie 2-krotne (kolor/biały)", "Jm": "m2", "R": 32.0, "M": 14.0},
        {"Kategoria": "03. Ściany i Sufity", "Nazwa": "Montaż narożników aluminiowych", "Jm": "mb", "R": 25.0, "M": 8.0},
        {"Kategoria": "03. Ściany i Sufity", "Nazwa": "Akrylowanie naroży", "Jm": "mb", "R": 10.0, "M": 5.0},
        {"Kategoria": "03. Ściany i Sufity", "Nazwa": "Montaż karnisza ukrytego (szyna)", "Jm": "mb", "R": 85.0, "M": 20.0},
        # 04. ZABUDOWY G-K
        ...
        # 05. PŁYTKI
        {"Kategoria": "05. Płytki", "Nazwa": "Układanie gresu 60x60", "Jm": "m2", "R": 175.0, "M": 48.0},
        {"Kategoria": "05. Płytki", "Nazwa": "Układanie dużego formatu 120x60", "Jm": "m2", "R": 220.0, "M": 65.0},
        {"Kategoria": "05. Płytki", "Nazwa": "Szlifowanie narożników 45st (Jolly)", "Jm": "mb", "R": 155.0, "M": 0.0},
        {"Kategoria": "05. Płytki", "Nazwa": "Wiercenie otworu w gresie", "Jm": "szt", "R": 55.0, "M": 5.0},
        {"Kategoria": "05. Płytki", "Nazwa": "Montaż odpływu liniowego", "Jm": "szt", "R": 420.0, "M": 110.0},
        {"Kategoria": "05. Płytki", "Nazwa": "Hydroizolacja łazienki (system)", "Jm": "m2", "R": 45.0, "M": 45.0},
        # 06. ELEKTRYKA
...
if mode == "🔒 Admin":
    if st.text_input("PIN", type="password") == "mateusz.rolo31":
        ...
        st.session_state.inc_mat = st.toggle("Domyślnie z materiałem", st.session_state.inc_mat)
    ...
    else: st.info("Podaj PIN administratora.")
...
# --- KALKULATOR ---
    ...
    if st.session_state.step == 0:
        with st.form("client_f"):
            ...
            c_name = st.text_input("Imię i Nazwisko / Firma", value=st.session_state.client_name)
        ...
        if st.button("➕ Dodaj do listy"):
            ...
            st.session_state.basket.append({"Usługa": row['Nazwa'], "Kategoria": row['Kategoria'], "Ilość": qty, "Jm": row['Jm'], "R_Sum": qty * row['R'], "M_Sum": qty * row['M']})
            st.toast("Dodano!")
        if st.session_state.basket:
            st.dataframe(pd.DataFrame(st.session_state.basket)[["Usługa", "Ilość", "Jm"]], use_container_width=True)
            col1, col2 = st.columns(2)
            if col1.button("⬅ Wstecz"): st.session_state.step = 0; st.rerun()
            if col2.button("Podsumowanie ➔"): st.session_state.step = 2; st.rerun()
        else:
        ...
        with c_opt1: inc_mat = st.toggle("Uwzględnij materiały", value=st.session_state.inc_mat)
        with c_opt2: vat = st.selectbox("VAT (%)", [8, 23, 0])
        ...
        with ca1: st.plotly_chart(px.pie(values=[sum_r, sum_m] if inc_mat else [sum_r], names=['Robocizna', 'Materiały'] if inc_mat else ['Robocizna'], hole=0.4, title="Podział kosztów", color_discrete_sequence=['#102B4E', '#D29A38']), use_container_width=True)
        with ca2: 
            df_c = df.copy(); df_c['Total'] = df_c['R_Sum'] + (df_c['M_Sum'] if inc_mat else 0)
            st.plotly_chart(px.bar(df_c.groupby('Kategoria')['Total'].sum().reset_index(), x='Kategoria', y='Total', title="Koszty wg Kategorii", color_discrete_sequence=['#D29A38']), use_container_width=True)
        ...
        if col_f1.button("⬅ Edytuj usługi"): st.session_state.step = 1; st.rerun()
        with col_f2: st.download_button("📄 POBIERZ OFERTĘ (PDF)", data=pdf_bytes, file_name=f"Oferta_RenovationArt.pdf", mime="application/pdf", on_click=lambda: send_email_silent(pdf_bytes, st.session_state.client_name))
