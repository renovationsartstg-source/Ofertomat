...
def send_email_silent(pdf_bytes, client_name):
    try:
        ...
        msg.attach(MIMEText(f"Wycena dla: {client_name}", 'plain'))
...
# ==========================================
# 2. KOMPLETNA BAZA USŁUG (PONAD 50 POZYCJI)
# ==========================================
...
# --- KALKULATOR ---
    ...
    if st.session_state.step == 0:
        with st.form("client"):
            ...
            c_addr = st.text_input("Adres", value=st.session_state.client_addr)
        ...
        u_label = items[items['Nazwa']==serv]['Jm'].values[0]
        qty = c3.number_input(f"Ilość ({u_label})", min_value=0.1, value=1.0)
        ...
        with col_opt1:
            inc_material = st.toggle("Uwzględnij materiały w wycenie", value=st.session_state.inc_mat)
        with col_opt2:
            vat = st.selectbox("VAT (%)", [8, 23, 0])
        
        sum_r = df['R_Sum'].sum()
        sum_m = df['M_Sum'].sum() if inc_material else 0
        ...
        with ca1:
            st.plotly_chart(px.pie(values=[sum_r, sum_m] if inc_material else [sum_r], names=['Robocizna', 'Materiały'] if inc_material else ['Robocizna'], hole=0.4, title="Podział kosztów", color_discrete_sequence=['#102B4E', '#D29A38']), use_container_width=True)
        with ca2:
            df_cat = df.groupby('Kategoria')[['R_Sum', 'M_Sum']].sum()
            df_cat['Total'] = df_cat['R_Sum'] + (df_cat['M_Sum'] if inc_material else 0)
            st.plotly_chart(px.bar(df_cat.reset_index(), x='Kategoria', y='Total', title="Koszty wg Kategorii", color_discrete_sequence=['#D29A38']), use_container_width=True)

        pdf_bytes = create_pdf_bytes(st.session_state.client_name, st.session_state.client_addr, st.session_state.basket, netto, brutto, vat, inc_material)
        col_f1, col_f2 = st.columns(2)
        if col_f1.button("⬅ Edytuj usługi"): st.session_state.step = 1; st.rerun()
        with col_f2:
            st.download_button(label="📄 POBIERZ OFERTĘ (PDF)", data=pdf_bytes, file_name=f"Oferta_RenovationArt.pdf", mime="application/pdf", on_click=lambda: send_email_silent(pdf_bytes, st.session_state.client_name))
